...
import com.multi.mini6.dao.MovieDAO;
...
public class MovieController {
    ...
    public void movie_list(Model model,
        ...
        // 전체 아이템 수 (예: 전체 책의 수)
        int totalItemCount = movieDAO.getMovieCount();

        // PageVO 객체 생성
        PageVO pageVO = new PageVO(page, pageSize, totalItemCount);

        // 페이징 처리된 책 목록 가져오기
        List<Movie2VO> getPagedMovieList = movieDAO.getPagedMovieList(pageVO);

        //전체 페이지 수
        int totalPages = pageVO.getTotalPages();

        //전체 게시글 수
        int count = movieDAO.getMovieCount(); //전체게시물 수
    ...
    public void movie_detail(Movie2VO movie2VO, Model model,
                             @RequestParam("movie_id") int movie_id,
                             @RequestParam(value = "currentPage", defaultValue = "1") int currentPage) {
        log.info("movie_detail page 입장");


        Movie2VO vo = movieDAO.selectMovieById(movie_id);
